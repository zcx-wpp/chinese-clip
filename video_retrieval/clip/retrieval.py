    ...
    for video_id, video_hits in grouped.items():
        ...
        for hit in video_hits:
            ...
            if current is not None:
                ...
                semantic_similarity = (
                    float(np.dot(previous_embedding, current_embedding))
            ...
            if current is None or not can_merge:
                ...
                current["adjacent_similarities"].append(
                    float(np.dot(previous_embedding, current_embedding))
                )
...
class VideoRetriever:
    ...
    def _load_video_item_genericness(self) -> dict[str, float]:
        ...
        genericness_by_segment_id = {
            item["segment_id"]: float(item.get("genericness_score") or 0.0) for item in records
        }
        self.video_item_genericness_cache = {
    ...
    def _recall_video_ids(self, query_embeddings: np.ndarray) -> list[str]:
        ...
        raw_video_results = self.video_index.search(
            query_embeddings, self.retrieval_config.video_recall_top_k
        )
        for result_list in raw_video_results:
            for item_id, score in result_list:
                ...
                adjusted_score = (
                    float(score)
                    - self.retrieval_config.video_genericness_penalty_weight
                    * genericness_by_item_id.get(item_id, 0.0)
                )
                score_by_video[video_id] = max(
                    score_by_video.get(video_id, float("-inf")), adjusted_score
                )
        ranked_videos = sorted(score_by_video.items(), key=lambda item: item[1], reverse=True)
        return [
            video_id
            for video_id, _ in ranked_videos[
                : self.retrieval_config.video_recall_candidate_pool_size
            ]
        ]
    ...
    def _recall_segment_ids(
        ...
        raw_segment_results = self.segment_index.search(
            query_embeddings, self.retrieval_config.segment_recall_top_k
        )
        for result_list in raw_segment_results:
            for segment_id, score in result_list:
                score_by_segment[segment_id] = max(
                    score_by_segment.get(segment_id, float("-inf")), float(score)
                )
        ...
        if not candidate_video_ids:
            ...
            return [
                segment_id
                for segment_id, _ in filtered_segments[
                    : self.retrieval_config.segment_recall_candidate_pool_size
                ]
            ]

        candidate_video_set = set(candidate_video_ids)
        candidate_segment_records = self.metadata_store.get_segment_records_by_video_ids(
            candidate_video_ids
        )
        ...
        return [
            segment_id
            for segment_id, _ in fallback_segments[
                : self.retrieval_config.segment_recall_candidate_pool_size
            ]
        ]
    ...
    def rerank_video_scores(
        ...
        for item in records:
            ...
            full_query_score = (
                float(np.max(full_query_embeddings @ frame_embedding))
                if full_query_embeddings.size
                else 0.0
            )
        ...
        for item in records:
            ...
            final_frame_score = (
                ...
                - self.retrieval_config.rerank_genericness_penalty_weight
                * segment_genericness_score
            )

...
def video_embedding_outputs_ready(
    store: MetadataStore,
    ordered_segments: list[tuple],
) -> bool:
    """True when every segment with frame candidates has segment + frame embeddings on disk."""
    segment_ids = [
        segment_path.stem
        for _idx, segment_path, _start_time, candidates in ordered_segments
        if candidates
    ]
    if not segment_ids:
        return True

    segment_records = store.get_segment_records(segment_ids)
    segment_by_id = {item["segment_id"]: item for item in segment_records}
    if len(segment_by_id) != len(segment_ids):
        return False

    for segment_id in segment_ids:
        if load_existing_embedding(segment_by_id[segment_id].get("embedding_path")) is None:
            return False

    frame_records = store.get_frame_records_by_segment_ids(segment_ids)
    frames_by_segment: dict[str, list[dict]] = {}
    for frame in frame_records:
        frames_by_segment.setdefault(frame["segment_id"], []).append(frame)

    for segment_id in segment_ids:
        frames = frames_by_segment.get(segment_id, [])
        if not frames:
            return False
        for frame in frames:
            if load_existing_embedding(frame.get("embedding_path")) is None:
                return False
    return True


def segment_representations_from_store(
    store: MetadataStore,
    video_id: str,
    ordered_segments: list[tuple],
    ...
    representations: list[SegmentRepresentation] = []
    for item in store.get_segment_records(segment_ids):
        if load_existing_embedding(item.get("embedding_path")) is None:
            continue
...
def refresh_representative_segments(
    ...
    for video_id in video_ids:
        ...
        store.update_segment_representative_flags(
            video_id=video_id, selected_segment_ids=[item.segment_id for item in selected]
        )

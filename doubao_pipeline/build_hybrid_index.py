...
from .caption_records import CaptionRecord, is_structured_record, normalize_caption_record, read_caption_jsonl
...
from .profile_paths import default_index_dir, default_metadata_db_path, resolve_path
...
def parse_args():
    parser = argparse.ArgumentParser(description="Build a local hybrid retrieval index from Doubao captions.")
    parser.add_argument("--captions-jsonl", default=str(DEFAULT_CAPTIONS_JSONL))
    parser.add_argument("--profile", help="Optional profile name for metadata.db and hybrid index output.")
    ...
    parser.add_argument("--limit", type=int, default=0, help="Only index the first N caption rows. 0 means all.")
    parser.add_argument(
...
def load_records(captions_jsonl: Path, store: MetadataStore, limit: int, allow_legacy_caption_only: bool) -> list[CaptionRecord]:
    ...
    if captions_jsonl.exists():
        ...
        records = [record for row in store.list_video_records() if (record := _record_from_video_row(row)) is not None]
...
def build_index(
    ...
    *,
    ...
    dense_texts = [build_dense_source_text(record) for record in records]
    print(
    ...
    vectors = embedder.encode_passages(dense_texts, progress_desc="encode BGE")
    embedder.save(index_dir)

    print(f"[step] writing search documents records={len(records)}", flush=True)
    iterator = _progress(
        zip(records, vectors, strict=True),
        total=len(records),
        desc="write search docs",
        unit="video",
    )
    for record, vector in iterator:
        payload = record.structured_caption or {
            "tags": record.tags,
            "description": record.description,
        }

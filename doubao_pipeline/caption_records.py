...
def normalize_caption_record(row: dict[str, Any]) -> CaptionRecord | None:
    ...
    duration_value = row.get("duration_seconds") if row.get("duration_seconds") is not None else row.get("duration")
    try:
    ...
    return CaptionRecord(
        ...
        created_at=str(row.get("created_at") or row.get("caption_updated_at") or "").strip() or None,
    )

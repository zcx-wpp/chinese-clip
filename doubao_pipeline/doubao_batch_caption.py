...
from .config import ARTIFACT_ROOT, DEFAULT_VIDEO_DIR, PIPELINE_ROOT, WORKSPACE_ROOT
from .metadata_store import MetadataStore
from .portable_paths import portable_path_text
from .profile_paths import default_metadata_db_path, resolve_path
...
DEFAULT_OUTPUT_PATH = ARTIFACT_ROOT / "doubao_video_captions.jsonl"
DEFAULT_PROMPT = (
    "你将看到按时间顺序抽取的一组视频关键帧。"
...
def parse_args():
    ...
    parser.add_argument("--profile", help="Named storage profile for syncing structured captions into metadata.db.")
    parser.add_argument("--metadata-db", help="Optional metadata.db path for syncing tags/description into retrieval database.")
    parser.add_argument("--no-sync-metadata-db", action="store_true", help="Disable writing structured captions into metadata.db.")
    parser.add_argument(
        "--video-id",
        dest="video_ids",
        action="append",
    ...
    parser.add_argument("--video-index-start", type=int, help="Only process videos whose numeric id is >= this value.")
    parser.add_argument("--video-index-end", type=int, help="Only process videos whose numeric id is <= this value.")
    ...
    parser.add_argument("--api-key-env", default=DEFAULT_API_KEY_ENV, help="Env var name for the Ark API key.")
    ...
    parser.add_argument("--prompt-file", help="Optional text file. Non-empty lines are joined with spaces.")
    parser.add_argument("--scene-scan-stride-seconds", type=float, default=DEFAULT_SCENE_SCAN_STRIDE_SECONDS)
    parser.add_argument("--scene-change-threshold", type=float, default=DEFAULT_SCENE_CHANGE_THRESHOLD)
    parser.add_argument(
...
def choose_keyframe_indices(
    ...
    if len(merged) < sample_frames:
        merged = downsample_sorted_indices(merged + uniform_sample_indices(total_frames, sample_frames), sample_frames)
    return downsample_sorted_indices(merged, sample_frames)


def sample_video_frames_fallback(capture: cv2.VideoCapture, sample_frames: int, max_side: int, jpg_quality: int) -> tuple[list[SampledFrame], float]:
...
def sample_video_frames(
    ...
    try:
        ...
        if total_frames <= 0:
            sampled, fallback_duration = sample_video_frames_fallback(capture, sample_frames, max_side, jpg_quality)
            if not sampled:
...
def request_caption_via_responses(config: CaptionConfig, frames: list[SampledFrame]) -> StructuredCaption:
...
def request_caption_via_chat(config: CaptionConfig, frames: list[SampledFrame]) -> StructuredCaption:
    client = get_client(config)
    ...
    if config.video_index_start is not None or config.video_index_end is not None:
        def in_index_range(path: Path) -> bool:
            ...
            if config.video_index_start is not None and index < config.video_index_start:
                return False
            if config.video_index_end is not None and index > config.video_index_end:
                return False
            return True
...
def run_batch(config: CaptionConfig):
    ...
    failure = 0
    started_at = time.time()
    metadata_store = MetadataStore(config.metadata_db_path) if config.sync_metadata_db and config.metadata_db_path is not None else None
    try:
        with ThreadPoolExecutor(max_workers=config.workers) as executor:
            future_map = {executor.submit(process_video, config, video_path): video_path for video_path in pending_videos}

...
class HybridSearchEngine:
    ...
    def _load_dense_state(self) -> None:
        ...
        vectors = [row["embedding_vector"] for row in rows if row.get("embedding_vector") is not None]
        if vectors:
            self.video_ids = [row["video_id"] for row in rows if row.get("embedding_vector") is not None]
    ...
    def search(
        ...
        *,
        ...
        for item in results:
            ...
            item["segments"] = [{"start": 0.0, "end": round(duration, 3), "score": round(float(item["score"]), 6)}]
        return results
        ...
        *,
        ...
        for source, engine in self.engines:
            ...
            for item in results:
                ...
                existing["score"] = float(existing.get("score") or 0.0) + float(item.get("score") or 0.0)
        ...
        for item in results:
            item["source_profiles"] = sorted(set(str(name) for name in item.get("source_profiles", [])))

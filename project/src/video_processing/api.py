...
def build_retriever(
    *,
    ...
    resolved_model_path = model_path or str(PROJECT_ROOT / "models")

    faiss_dir = output_dir / "faiss"
    frame_index = FaissFrameIndex.load(faiss_dir / "frame_index.faiss", faiss_dir / "frame_index.meta.json")
    segment_index = FaissFrameIndex.load(faiss_dir / "segment_index.faiss", faiss_dir / "segment_index.meta.json")
    video_index = None
    ...
    retrieval_config = replace(RetrievalConfig(), **{k: v for k, v in overrides.items() if v is not None})

    encoder = ChineseClipEncoder(model_path=resolved_model_path, device=device, batch_size=batch_size)

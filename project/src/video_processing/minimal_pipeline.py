...
def process_video(config: MinimalConfig, video_path: Path, video_idx: int, total_videos: int) -> tuple[str, bool]:
    ...
    stage_log(video_id, "init", device=config.device, video_workers=config.video_workers, frame_workers=frame_extract_workers(config))
    encoder = get_encoder(config)
    stage_log(video_id, "encoder_ready", embedding_dim=encoder.embedding_dim, batch_size=config.batch_size)
    ...
    try:
        ...
        if segment_task_status != "done":
            ...
            try:
                ...
                stage_log(video_id, "segment", segments=len(segment_paths), seconds=config.segment_seconds)
        ...
        if frame_task_status == "done":
            ...
            if not scheduler.frame_outputs_ready(config.frames_dir, video_id, segment_ids, config.image_format):
            ...
            try:
                if worker_count == 1:
                    for segment_idx, segment_path in enumerate(segment_paths):
                        segment_id = segment_path.stem
                        start_time = segment_idx * config.segment_seconds
                        end_time = min(duration, start_time + config.segment_seconds) if duration > 0 else start_time + config.segment_seconds
        ...
        for _, segment_path, start_time, candidates in ordered_segments:
            ...
            if not candidates:
                embedded_segments += 1
                stage_log(video_id, "embedding", progress=f"{embedded_segments}/{total_segments}", segment_id=segment_id, skipped="no_candidates")
                continue
            ...
            segment_matrix = np.vstack([item.embedding.astype(np.float32) for item in selected]).astype(np.float32)
        ...
        print(f"[done] video_id={video_id} representative_segments={len(selected_representations)}", flush=True)
        return video_id, True
    except Exception as exc:
...
def finalize_indexes(config: MinimalConfig, video_ids: list[str]):
    ...
    try:
        ...
        persist_faiss_index(
            ...
            segment_ids,
            segment_embedding_rows,
            *config.faiss_paths("segment_index"),
        )
        ...
        persist_faiss_index(
            ...
            video_item_ids,
            video_embedding_rows,
            *config.faiss_paths("video_index"),
        )

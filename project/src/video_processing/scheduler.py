...
class PipelineScheduler:
    ...
    def _update_task(self, video_id: str, stage: str, status: str, *, error_message: str | None = None, increment_retry: bool = False):
    ...
    def _mark_video(self, video_id: str, status: str, *, error_message: str | None = None):
        self.store.mark_video_status(video_id, status=status, error_message=error_message, updated_at=self._now())
    ...
    def fail_task(self, video_id: str, stage: str, error_message: str):
        self._update_task(video_id, stage, "failed", error_message=error_message, increment_retry=True)
    ...
    def frame_outputs_ready(self, frames_dir: Path, video_id: str, segment_ids: list[str], image_format: str) -> bool:
        if not segment_ids:
            return False

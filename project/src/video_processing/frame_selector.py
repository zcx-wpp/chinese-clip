...
def _initialize_kmeans_centers(embeddings: np.ndarray, norms: np.ndarray, cluster_count: int) -> np.ndarray:
...
def _run_kmeans(embeddings: np.ndarray, norms: np.ndarray, cluster_count: int, iterations: int) -> tuple[np.ndarray, np.ndarray]:
...
def select_top_k_frames(
    ...
    for cluster_idx in range(cluster_count):
        ...
        if cluster_indices.size == 0:
            continue
        for candidate_idx in _cluster_candidate_order(cluster_indices, centers[cluster_idx], embeddings, norms):
            candidate_embedding = embeddings[candidate_idx]
            if selected_embeddings:
                sims = [_cosine_similarity(candidate_embedding, other) for other in selected_embeddings]
